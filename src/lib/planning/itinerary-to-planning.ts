...
export function itineraryToPlanningItems(itinerary: Itinerary | null | undefined): PlanningItem[] {
  ...
  itinerary.route.bases.forEach((base) => {
    ...
    if (!citySet.has(cityName)) {
      ...
      items.push({
        ...
        imageUrl: `/api/placeholder/city/${encodeURIComponent(cityName)}`,
  ...
  itinerary.route.bases.forEach((base) => {
    if (base.accommodation?.name) {
      items.push({
        ...
        imageUrl: `/api/placeholder/city/${encodeURIComponent(base.location)}`,
        category: 'hotels',
    ...
    day.blocks.forEach((block, blockIndex) => {
      if (block.activity) {
        ...
        items.push({
          ...
          imageUrl: `/api/placeholder/city/${encodeURIComponent(activity.name)}`,
          category,
          rating: undefined,
  ...
  itinerary.foodLayer.forEach((food) => {
    ...
    items.push({
      ...
      imageUrl: `/api/placeholder/city/${encodeURIComponent(food.name)}`,
