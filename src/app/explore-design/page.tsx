...
// Static design mockup - not functional, just for UI review
...
export default function ExploreDesignPage() {
  ...
  const upcomingTrips = [
    { name: 'Thailand', saves: 12, lastViewed: '2d ago', image: '/api/placeholder/city/Bangkok', slug: 'thailand' },
    { name: 'Japan', saves: 8, lastViewed: '5d ago', image: '/api/placeholder/city/Tokyo', slug: 'japan' },
    { name: 'Hawaii', saves: 6, lastViewed: '3d ago', image: '/api/placeholder/city/Honolulu', slug: 'hawaii' },
    { name: 'Vietnam', saves: 5, lastViewed: '1w ago', image: '/api/placeholder/city/Hanoi', slug: 'vietnam' },
  ];

  // Beach vacation destinations
  const beachDestinations = [
    { name: 'Thailand', subtitle: 'Phuket, Krabi, Koh Samui', image: '/api/placeholder/city/Phuket', region: 'Asia', slug: 'thailand' },
    { name: 'Indonesia', subtitle: 'Bali, Lombok, Gili Islands', image: '/api/placeholder/city/Bali', region: 'Asia', slug: 'indonesia' },
    { name: 'Maldives', subtitle: 'Overwater villas, crystal waters', image: '/api/placeholder/city/Maldives', region: 'Asia', slug: 'maldives' },
    { name: 'Greece', subtitle: 'Santorini, Mykonos, Crete', image: '/api/placeholder/city/Santorini', region: 'Europe', slug: 'greece' },
    { name: 'Mexico', subtitle: 'Cancun, Tulum, Playa del Carmen', image: '/api/placeholder/city/Cancun', region: 'Americas', slug: 'mexico' },
  ];

  // History & culture destinations
  const historyDestinations = [
    { name: 'Italy', subtitle: 'Rome, Florence, Venice', image: '/api/placeholder/city/Rome', region: 'Europe', slug: 'italy' },
    { name: 'Greece', subtitle: 'Athens, Delphi, Olympia', image: '/api/placeholder/city/Athens', region: 'Europe', slug: 'greece' },
    { name: 'Egypt', subtitle: 'Pyramids, Luxor, Valley of Kings', image: '/api/placeholder/city/Cairo', region: 'Africa', slug: 'egypt' },
    { name: 'Peru', subtitle: 'Machu Picchu, Cusco, Lima', image: '/api/placeholder/city/Cusco', region: 'Americas', slug: 'peru' },
    { name: 'Japan', subtitle: 'Kyoto, Nara, Hiroshima', image: '/api/placeholder/city/Kyoto', region: 'Asia', slug: 'japan' },
  ];

  // Food destinations
  const foodDestinations = [
    { name: 'Japan', subtitle: 'Sushi, Ramen, Kaiseki', image: '/api/placeholder/city/Tokyo', region: 'Asia', slug: 'japan' },
    { name: 'Thailand', subtitle: 'Street food, Pad Thai, Tom Yum', image: '/api/placeholder/city/Bangkok', region: 'Asia', slug: 'thailand' },
    { name: 'Italy', subtitle: 'Pasta, Pizza, Gelato', image: '/api/placeholder/city/Naples', region: 'Europe', slug: 'italy' },
    { name: 'Vietnam', subtitle: 'Pho, Banh Mi, Fresh rolls', image: '/api/placeholder/city/Hanoi', region: 'Asia', slug: 'vietnam' },
    { name: 'Mexico', subtitle: 'Tacos, Mole, Street corn', image: '/api/placeholder/city/MexicoCity', region: 'Americas', slug: 'mexico' },
  ];
        ...
        <section>
          ...
          <div className="grid grid-cols-4 gap-2">
            {upcomingTrips.map((trip) => (
              <button
                key={trip.name}
                onClick={() => router.push(`/explore-design/${trip.slug}`)}
                className="relative rounded-xl overflow-hidden shadow-sm text-left"
              >
                <div className="aspect-[3/4] bg-gradient-to-br from-blue-200 to-purple-200">
                  <img src={trip.image} alt={trip.name} className="w-full h-full object-cover" />
                  <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-black/20 to-transparent" />
                </div>
                <div className="absolute bottom-0 left-0 right-0 p-2 text-white">
                  <h3 className="font-semibold text-xs">{trip.name}</h3>
                  <p className="text-[10px] text-white/80">{trip.saves} Saves</p>
                </div>
              </button>
            ))}
          </div>
        ...
        <section>
          ...
          <div className="grid grid-cols-4 gap-2">
            {filterByRegion(beachDestinations, beachFilter).slice(0, 4).map((dest) => (
              <button
                key={dest.name}
                onClick={() => router.push(`/explore-design/${dest.slug}`)}
                className="relative rounded-xl overflow-hidden shadow-sm text-left"
              >
                <div className="aspect-[3/4] bg-gradient-to-br from-cyan-200 to-blue-300">
          ...
          <div className="grid grid-cols-4 gap-2">
            {filterByRegion(historyDestinations, historyFilter).slice(0, 4).map((dest) => (
              <button
                ...
                className="relative rounded-xl overflow-hidden shadow-sm text-left"
              >
                <div className="aspect-[3/4] bg-gradient-to-br from-amber-200 to-orange-300">
                  <img src={dest.image} alt={dest.name} className="w-full h-full object-cover" />
                  <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-black/20 to-transparent" />
                </div>
                <div className="absolute bottom-0 left-0 right-0 p-2 text-white">
          ...
          <div className="grid grid-cols-4 gap-2">
            {filterByRegion(foodDestinations, foodFilter).slice(0, 4).map((dest) => (
              <button
                ...
                className="relative rounded-xl overflow-hidden shadow-sm text-left"
              >
                <div className="aspect-[3/4] bg-gradient-to-br from-red-200 to-pink-300">
                  <img src={dest.image} alt={dest.name} className="w-full h-full object-cover" />
                  <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-black/20 to-transparent" />
                </div>
                <div className="absolute bottom-0 left-0 right-0 p-2 text-white">
