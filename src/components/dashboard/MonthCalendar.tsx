...
export function MonthCalendar({ trips, onDateClick, compact = false, itinerary, contentFilter, collapsible = false, defaultCollapsed = false }: MonthCalendarProps) {
  ...
  const getCategoryColor = (category: string) => {
    switch (category) {
      case 'transport': return 'bg-blue-500';    // Blue for Transport
      case 'hotel': return 'bg-purple-500';      // Purple for Hotels
      case 'food': return 'bg-orange-500';       // Orange for Food
      case 'activity': return 'bg-yellow-500';   // Yellow for Activities
